...
class Experiment:
    ...
    def register_experiment(self, force=False, mode = "mean", sum=False, n_jobs=-1, fiduciary = None, parallel_time = False, y_lims = (0,-1), x_lims = (0,-1), plot = False, remove_extracted=False):
        ...
        if fiduciary:
            ...
            Parallel(n_jobs=-1)(delayed(self.warp_and_save)(FOV, channel, time, tmat, remove_extracted) for time, tmat in zip(self.times, self.tmats) for FOV in tqdm(self.FOVs) for channel in self.channels)
        else:
            for FOV in tqdm(self.FOVs):
                tmats = self.get_transformation_matrices(FOV, mode, y_lims, x_lims, plot = plot)
                Parallel(n_jobs=-1)(delayed(self.warp_and_save)(FOV, channel, time, tmat, remove_extracted) for time, tmat in zip(self.times, tmats) for channel in self.channels)
    ...
    def warp_and_save(self, FOV, channel, time, tmats, remove_extracted=False):
        # Path to the unregistered file in /extracted
        src_path = self.img_path_generator(
            self.extracted_dir, FOV, channel, time, self.file_extension
        )
    
        # Path where the registered image will be saved
        out_path = self.img_path_generator(
            self.registered_dir, FOV, channel, time, self.save_file_extension
        )
    
        # 1. and 2. Warp the image
        mov = self.warp_image(FOV, channel, time, tmats)
        # 3. Save to /registered
        self.imwriter(out_path, mov)
    
        # 4. Remove the original unregistered file IF requested
        if remove_extracted:
            # Only remove if the new file actually exists (i.e., was saved correctly)
            if os.path.exists(out_path):
                try:
                    os.remove(src_path)
                except OSError as e:
                    warnings.warn(f"Could not remove {src_path}. Error: {e}")
            else:
                warnings.warn(f"Registered file {out_path} not found, so {src_path} was not removed.")
...
def visualize_crop(input_directory, crop_top, crop_bottom, num_lanes, flip_odd, rotate_angle=0.0):
    ...
    if image_file:
        ...
        # Visualize the subimages dynamically based on num_lanes
        cols = 3
        rows = int(np.ceil(num_lanes / cols))
        fig, ax = plt.subplots(rows, cols, figsize=(15, 5 * rows))
        
        # Flatten the axes array for easy indexing, handle case where there's only 1 subplot
        if num_lanes > 1:
            ax = ax.flatten()  
        else:
            ax = [ax] 
        ...
        for i in range(len(ax)):
            if i < len(subimages):
                ax[i].imshow(subimages[i], cmap='gray')
                ax[i].set_title(f"Subimage {i+1}")
            ax[i].axis('off')  # Hide axis for all plots, including empty ones
...
# The pattern moves with the functions
pattern = re.compile(r'xy(\d+)_(.+)_T(\d+)\.png')

def process_image(image_file, input_directory, output_directory, delete_original, num_lanes, crop_top, crop_bottom, flip_odd):
...
def split_and_save_images(input_directory, output_directory, delete_original, num_lanes, crop_top, crop_bottom, flip_odd, n_jobs=-1):
    os.makedirs(output_directory, exist_ok=True)
    image_files = sorted([f for f in os.listdir(input_directory) if f.endswith('.png')])
    
    # n_jobs is now passed directly to Parallel
    Parallel(n_jobs=n_jobs)(
        delayed(process_image)(
            image_file, input_directory, output_directory, delete_original, num_lanes, crop_top, crop_bottom, flip_odd
        )
